...
nplots = numel(out{1});
data = nan(nwindows,nplots,2); % dim3: [mean, sem]
for i_w = 1:nwindows
    for i_p = 1:nplots
        thisplot = out{i_w}{i_p};
        if isempty(thisplot); continue; end
        data(i_w,i_p,1) = mean(thisplot.data,'omitmissing'); % mean
        data(i_w,i_p,2) = std(thisplot.data,[],'omitmissing')/numel(thisplot.data);
    end
end
...
y1 = data(:,idx,1);
[~,tend1] = max(y1);
[params1, ~, ~] = fitExpSaturation(t(tstart:tend1), y1(tstart:tend1), 0);

idx = find(idx_by_stimgroup==7 & idx_by_subjectgroup==2); % all novel & trained
y2 = data(:,idx,1);
[~,tend2] = max(y2);
[params2, ~, ~] = fitExpSaturation(t(tstart:tend2), y2(tstart:tend2), 0);

idx = find(idx_by_stimgroup==7 & idx_by_subjectgroup==1); % all novel & naive
y3 = data(:,idx,1);
[~,tend3] = max(y3);
[params3, ~, ~] = fitExpSaturation(t(tstart:tend3), y3(tstart:tend3), 0);

hf = figure;
set(gcf, 'color', cfg.bgcol); 

subplot(121); hold on
b(1) = plot(t(tstart:tend1), y1(tstart:tend1));
b(2) = plot(t(tstart:tend2), y2(tstart:tend2));
b(3) = plot(t(tstart:tend3), y3(tstart:tend3));
axis tight
...
xticklabels({'amplitude','tau','offset'})
box off
ylabel('Fitted value');
set(gca, 'color', cfg.bgcol, 'XColor',cfg.axcol, 'YColor',cfg.axcol, 'ZColor',cfg.axcol);
