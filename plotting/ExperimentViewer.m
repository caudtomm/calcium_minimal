classdef ExperimentViewer
    ...
    methods
        ...
        function [out,all_labs] = getModeDecomposition(obj, varargin)
            ...
            TuningSelectivityZero = []; % tuning selectivity (shuffled avg)
            ...
            if do_pool
                ...
                if ~isempty(GSS)
                    GSS = cell2mat(GSS);
                    TuningSelectivity = cell2mat(TuningSelectivity);
                    TuningSelectivityZero = cell2mat(TuningSelectivityZero);
                end
                ...
                all_out = doModeDecomposition(data, ...
                        'method', method, ...
                        'nfactors', nfactors, ...
                        'suppression score',GSS, ...
                        'selectivity of tuning',TuningSelectivity, ...
                        'SoT_zero', mean(TuningSelectivityZero,'omitmissing'));
            else
                all_out = cell(nsubjects,1);
                for i = 1:nsubjects
                    thisevents = events{i};
